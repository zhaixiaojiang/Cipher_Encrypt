...
public enum DesEnum {
	...
	private DesEnum(String encryptType) {
	...
	public static DesEnum getMode(String encryptionMode){
		DesEnum[] values = DesEnum.values();
		for (DesEnum value : values) {
			if (value.getEncryptType().equalsIgnoreCase(encryptionMode)) {
				return value;
			}
		}
		return DesEnum.ECB_PKCS5PADDING;
	}
}
